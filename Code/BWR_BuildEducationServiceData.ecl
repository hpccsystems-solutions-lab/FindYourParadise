...
EdScoreDS := DATASET('~BMF::Hackathon::EducationSummary',EducationScoreRec,THOR);
EdScoreDS;
...
AddSTScore  := ITERATE(SORT(TempTbl,-avestratio),TRANSFORM(RankTbl,SELF.StudentTeacherScore := IF(LEFT.avestratio=RIGHT.avestratio,LEFT.StudentTeacherScore,LEFT.StudentTeacherScore+1),SELF := RIGHT));
...
OUTPUT(AddPubScore,,'~BMF::hackathon::EducationScores',NAMED('TopFat'),OVERWRITE);
